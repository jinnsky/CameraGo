...
open class SCCameraViewController: UIViewController {
    ...
    public weak var cameraDelegate: SCCameraViewControllerDelegate?
    ...
    public var videoGravity: SCCameraPreviewViewVideoGravity = .resizeAspect
    ...
    fileprivate var previewLayer: SCCameraPreviewView!
    ...
    override open func loadView() {
        view = UIView(frame: UIScreen.main.fixedCoordinateSpace.bounds)
        
        previewLayer = SCCameraPreviewView(frame: view.frame, videoGravity: videoGravity)
    ...
    public func startVideoRecording() {
        ...
        sessionQueue.async { [unowned self] in
            if !movieFileOutput.isRecording {
                ...
                DispatchQueue.main.async {
                    self.cameraDelegate?.cameraViewController(self, didBeginRecordingVideo: self.currentCamera)
                }
            }
            else {
    ...
    public func stopVideoRecording() {
        if self.movieFileOutput?.isRecording == true {
            ...
            DispatchQueue.main.async {
                self.cameraDelegate?.cameraViewController(self, didFinishRecordingVideo: self.currentCamera)
            }
        }
    }
    ...
    public func switchCamera() {
        guard isVideoRecording != true else {
            ...
            print("Switching between cameras while recording video is not supported")
            return
        }
        ...
        sessionQueue.async { [unowned self] in
            ...
            self.addInputs()
            DispatchQueue.main.async {
                self.cameraDelegate?.cameraViewController(self, didSwitchCameras: self.currentCamera)
            }
    ...
    fileprivate func addVideoInput() {
        switch currentCamera {
        case .front:
            videoDevice = SCCameraViewController.deviceWithMediaType(AVMediaType.video.rawValue, preferringPosition: .front)
        case .rear:
            videoDevice = SCCameraViewController.deviceWithMediaType(AVMediaType.video.rawValue, preferringPosition: .back)
        }
    ...
    fileprivate func processPhoto(_ imageData: Data) -> UIImage {
        ...
        return UIImage(cgImage: cgImageRef!, scale: 1.0, orientation: self.getImageOrientation(forCamera: self.currentCamera))
    }
        ...
        if let videoConnection = photoFileOutput?.connection(with: AVMediaType.video) {
            ...
            photoFileOutput?.captureStillImageAsynchronously(from: videoConnection, completionHandler: {(sampleBuffer, error) in
                if (sampleBuffer != nil) {
                    ...
                    DispatchQueue.main.async {
                        self.cameraDelegate?.cameraViewController(self, didTake: image)
                    }
    ...
    fileprivate func videoInputPresetFromVideoQuality(quality: VideoQuality) -> String {
        ...
        case .resolution3840x2160:
            if #available(iOS 9.0, *) {
                ...
                print("Resolution 3840x2160 not supported")
    ...
    fileprivate class func deviceWithMediaType(_ mediaType: String, preferringPosition position: AVCaptureDevice.Position) -> AVCaptureDevice? {
        return AVCaptureDevice.devices(for: AVMediaType(rawValue: mediaType)).filter({ $0.position == position }).first
    }
...
extension SCCameraViewController : AVCaptureFileOutputRecordingDelegate {
    ...
    public func fileOutput(_ output: AVCaptureFileOutput, didFinishRecordingTo outputFileURL: URL, from connections: [AVCaptureConnection], error: Error?) {
        ...
        if error != nil {
            ...
            DispatchQueue.main.async {
                self.cameraDelegate?.cameraViewController(self, didFailToRecordVideo: error!)
            }
        } else {
            //Call delegate function with the URL of the outputfile
            DispatchQueue.main.async {
                self.cameraDelegate?.cameraViewController(self, didFinishProcessVideoAt: outputFileURL)
            }
        }
    }
}
...
extension SCCameraViewController {
    @objc fileprivate func zoomGesture(pinch: UIPinchGestureRecognizer) {
        ...
        do {
            ...
            DispatchQueue.main.async {
                self.cameraDelegate?.cameraViewController(self, didChangeZoomLevel: self.zoomScale)
            }
    ...
    @objc fileprivate func singleTapGesture(tap: UITapGestureRecognizer) {
        ...
        if let device = videoDevice {
            do {
                ...
                DispatchQueue.main.async {
                    self.cameraDelegate?.cameraViewController(self, didFocusAtPoint: tapPoint)
                }
            }
            catch {
                // just ignore
...
extension SCCameraViewController : UIGestureRecognizerDelegate {
